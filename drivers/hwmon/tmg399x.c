...
#define PROX_THRESHOLD_DISTANCE             40
...
#define PROX_DATA_SAFE_RANGE_MAX            (PROX_DATA_TARGET*2 + 10)
...
#define PROX_THRESHOLD_HIGH_MAX             150
#define PROX_THRESHOLD_HIGH_MIN             75
...
static void tmg399x_chip_data_init(struct tmg399x_chip *chip)
{
    ...
    chip->prox_thres_lo_min                 = PROX_THRESHOLD_HIGH_MIN;
    ...
    if (gesflvl>0)
    {
    	...
    	if ( (gstatus & TMG399X_GES_ST_MASK) == TMG399X_GES_VALID)
        {
            ...
            for(i=0; i<(gesflvl-1); i++)
            {
                ...
                if (chip->ges_debug_enable || chip->gesture_offset_cal_start)
                {
