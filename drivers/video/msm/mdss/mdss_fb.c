...
#ifdef CONFIG_ZTEMT_LCD_DISP_ENHANCE
//because max_light must 480,so bl must be 4095,the code can close
/*add init code second part,mayu add 3.5*/
   struct zte_enhance_type enhance_val;

   enhance_val = zte_get_lcd_enhance_param();
  //printk("lcd:%s en_colortmp=%d ,maxbl=%d\n",__func__,enhance_val.en_colortmp,enhance_val.cur_bl_max);

   if((enhance_val.en_colortmp) && (enhance_val.cur_bl_max!= mfd->panel_info->bl_max))
   {
      printk("lcd:%s change bl_max from %d to %d\n",__func__,mfd->panel_info->bl_max,enhance_val.cur_bl_max);
      mfd->panel_info->bl_max = enhance_val.cur_bl_max;
   }
#endif
