...
#ifdef NXP_TPS65132_DEBUG
	printk("[tps]%s:positive_voltage=%d,negative_voltage=%d, ",__FUNCTION__,positive_voltage,negative_voltage);
...
void  tps65132_show_reg(void )
{
	int val0,val1,regff_val;
	 ...
	 regff_val=tps65132_read_reg(0xff);
#ifdef NXP_TPS65132_DEBUG
	printk("[tps]%s:reg00=0x%x, reg01=0x%x ,regff_val= 0x%x~~~~~~~~~~~~\n",__FUNCTION__,val0,val1,regff_val);
#endif	
}
...
//I2C POWER
  ...
  if (ret)
    ...
    tps65132_set_output_voltage(5600,5400);
#else
