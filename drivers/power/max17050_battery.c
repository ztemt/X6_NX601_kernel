...
extern int qpnp_get_batt_health(void);
extern int qpnp_get_batt_stauts(void);
...
{
	return qpnp_get_batt_stauts();
}

int max17050_get_prop_batt_health(void)
{
	return qpnp_get_batt_health();
}
int max17050_get_property(struct power_supply *psy,
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = max17050_get_batt_temp();
		break;
...
EXPORT_SYMBOL(max17050_get_property);
...
static void max17050_batt_worker(struct work_struct *work)
{
	...
	batt_vol = max17050_get_batt_voltage()/1000;
	...
	if(last_batt_soc >= 0){
		if(chip->first_resume){
            ...
            chip->first_resume = 0;
		}else if( last_batt_soc < batt_soc  )
			last_batt_soc++;
		else if( last_batt_soc > batt_soc  )
			last_batt_soc--;
	}else
	...
	if(batt_soc != chip->batt_soc)
		update_power_supply(chip);

	MAXLOG_INFO("BATT soc=%d temp=%d i=%d vol=%d \n",chip->batt_soc,batt_temp,batt_i,batt_vol);

	if (MAX_DEBUG < maxlog_level)
		max17050_dump_regs(chip);

	schedule_delayed_work(&chip->batt_worker,
			  round_jiffies_relative(msecs_to_jiffies(BATT_MONITOR_MS)));
}
