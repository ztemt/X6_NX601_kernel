...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	host_mode = qpnp_chg_is_otg_en_set(chip);
	#ifdef  CONFIG_ZTEMT_CHARGE_QPNP
	PMLOG_DEBUG("usbin-valid triggered: %d host_mode: %d\n",
...
qpnp_chg_bat_if_batt_temp_irq_handler(int irq, void *_chip)
{
	...
	#ifdef  CONFIG_ZTEMT_CHARGE_QPNP
	PMLOG_DEBUG("batt-temp triggered: %d\n", batt_temp_good);
    #endif
	pr_debug("psy changed batt_psy\n");
	power_supply_changed(&chip->batt_psy);
	return IRQ_HANDLED;
...
int qpnp_get_batt_stauts(void)
{
    ...
    return get_prop_batt_status(qpnp_chip);
}

int qpnp_get_batt_health(void)
{
	if (!qpnp_chip) {
		pr_err("%s:called before init\n",__func__);
		return 0;
	}
    return get_prop_batt_health(qpnp_chip);
}
int qpnp_is_usb_present(void)
{
    ...
    return qpnp_chg_is_usb_chg_plugged_in(qpnp_chip) ;
}
...
qpnp_charger_probe(struct spmi_device *spmi)
{
 ...
 qpnp_chg_state_machine_reset(chip);
	...
	if (chip->bat_if_base) {
		...
		#ifdef CONFIG_ZTEMT_BATTERY_MAX17050
        if(1){
			chip->batt_psy.properties = max17050_props;
			chip->batt_psy.num_properties = ARRAY_SIZE(max17050_props);
			chip->batt_psy.get_property = max17050_get_property;
		}else{
