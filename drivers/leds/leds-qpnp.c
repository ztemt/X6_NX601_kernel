...
#ifdef CONFIG_ZTEMT_WLED_CABC
#define WLED_CABC_ENABLE_DEFAULT	0x0
#endif
...
#define ZTEMT_RGB_BREATH_LED_FULL    100
...
#ifdef CONFIG_ZTEMT_RGB_BREATH_LEDS
u32 ztemt_lpg_pm_pwm_period=1000;
#endif
...
enum rgb_led_mode {
	...
	rgb_led_mode_blink_1,
	rgb_led_mode_blink_2,
	rgb_led_mode_blink_3 ,
	rgb_led_mode_blink_4,
	rgb_led_mode_blink_5,
};
u32 rgb_red_mode;
u32 rgb_green_mode;
u32 rgb_blue_mode;
int rgb_red_current_brightness;
int rgb_green_current_brightness;
int rgb_blue_current_brightness;
#endif
	...
	if (led->cdev.brightness) {
		...
		if (led->rgb_cfg->pwm_cfg->mode == PWM_MODE) {
            ...
            duty_us = (led->rgb_cfg->pwm_cfg->pwm_period_us *
				led->cdev.brightness) / ZTEMT_RGB_BREATH_LED_FULL;
...
/*mayu add 2013.9.22*/
		rc = qpnp_led_masked_write(led,
			WLED_CABC_REG(led->base, i), WLED_NO_MASK,led->cabc_on);
		if (rc) {
			dev_err(&led->spmi_dev->dev,
				"WLED max current reg write failed(%d)\n", rc);
			return rc;
	...
	const char *buf, size_t count)
{
    ...
    struct pwm_config_data *pwm_cfg;
    char *buffer;
    //int max_duty_pcts = PWM_LUT_MAX_SIZE;
    int num_duty_pcts = 0;
    int max_parameter=10;
    //int num_parameter=0;
    ...
    //ret = kstrtoul(buf, 10, &ramp_step_ms);
    //ret = kstrtoul(buf, 10, &start_idx);
    //ret = kstrtoul(buf, 10, &pause_hi);
    //ret = kstrtoul(buf, 10, &pause_lo);
    //ret = kstrtoul(buf, 10, &lut_flags);
    ...
    switch (led->id) {
	  case QPNP_ID_RGB_RED:
		   rgb_red_mode=mode;
		   rgb_red_current_brightness=brightness;
		   pwm_cfg = led->rgb_cfg->pwm_cfg;
		   break;
	  case QPNP_ID_RGB_GREEN:
		   rgb_green_mode=mode;
		   rgb_green_current_brightness=brightness;
		   pwm_cfg = led->rgb_cfg->pwm_cfg;
		   break;
	  case QPNP_ID_RGB_BLUE:
		   rgb_blue_mode=mode;
		   rgb_blue_current_brightness=brightness;
		   pwm_cfg = led->rgb_cfg->pwm_cfg;
		   break;
	  default:
	      dev_err(&led->spmi_dev->dev, "Invalid LED id type for blink\n");
	...
	if(mode==rgb_led_mode_normal) //normal brightness
	{
	...
	else if(mode==rgb_led_mode_blink_1)  //breath blink
	{
		ztemt_rgb_led_debug("%s : %d : rgb_led_mode_blink_1\n",__func__,__LINE__);
		//current_brightness=led->cdev.brightness;
		...
		for(j=0;j<PWM_LUT_MAX_SIZE;j++)
		{
		    ...
		    num_duty_pcts++;
		    if(temp_brightness>=ZTEMT_RGB_BREATH_LED_FULL){
			    temp_brightness=ZTEMT_RGB_BREATH_LED_FULL;
	...
	else if(mode==rgb_led_mode_blink_2)  //charge breath
	{
		ztemt_rgb_led_debug("%s : %d : rgb_led_mode_blink_2\n",__func__,__LINE__);
		//current_brightness=led->cdev.brightness;
		current_brightness=brightness;
		temp_brightness=current_brightness;
		num_duty_pcts=0;
	...
	else if(mode==rgb_led_mode_blink_3) //power on
	{
		ztemt_rgb_led_debug("%s : %d : rgb_led_mode_blink_3\n",__func__,__LINE__);
		//current_brightness=led->cdev.brightness;
		current_brightness=brightness;
		temp_brightness=0;
		num_duty_pcts=0;
     ...
     */		
		...
		for(j=0;j<PWM_LUT_MAX_SIZE;j++)
		{
		 if(temp_brightness>=current_brightness){
				temp_brightness=current_brightness;
				pwm_cfg->old_duty_pcts[j] = temp_brightness;
		    num_duty_pcts++;
				break;
	...
	else if(mode==rgb_led_mode_blink_4) //power off
	{
		ztemt_rgb_led_debug("%s : %d : rgb_led_mode_blink_4\n",__func__,__LINE__);
		//current_brightness=led->cdev.brightness;
		...
		for(j=100;j>=0;j--)
		{
			if(temp_brightness<=0){
				pwm_cfg->old_duty_pcts[100-j] = 0;
				num_duty_pcts++;
				break;
			}
			pwm_cfg->old_duty_pcts[100-j] = temp_brightness;	
		    num_duty_pcts++;
			temp_brightness=temp_brightness-brightness_step;
		}
     */		
		...
		for(j=current_brightness;j>=0;j--)
		{
			if(temp_brightness<=0){
				pwm_cfg->old_duty_pcts[current_brightness-j] = 0;
				num_duty_pcts++;
				break;
			}
			pwm_cfg->old_duty_pcts[current_brightness-j] = temp_brightness;	
		  num_duty_pcts++;
			temp_brightness=temp_brightness-brightness_step;
		}
	...
	else if(mode==rgb_led_mode_blink_5)    //normal_brightness_2
	{
		ztemt_rgb_led_debug("%s : %d : rgb_led_mode_blink_5\n",__func__,__LINE__);
		//current_brightness=led->cdev.brightness;
		current_brightness=brightness;
		temp_brightness=current_brightness;
		num_duty_pcts=0;
	...
	*/
	...
	if (ret) {
	   ...
	   qpnp_led_set(&led->cdev, led->cdev.brightness);
	   dev_err(&led->spmi_dev->dev,
		"Failed to initialize pwm with new lpg value\n");
	   return ret;
    }	
	qpnp_led_set(&led->cdev, led->cdev.brightness);
	return count;
	...
	struct device_attribute *attr, char *buf)
{
	...
	u32 pause_hi=0;
	u32 ramp_step_ms=0;
	u32 lpg_pm_pwm_period;
	
	led = container_of(led_cdev, struct qpnp_led_data, cdev);
	switch (led->id) {	
	case QPNP_ID_RGB_RED:
		mode=rgb_red_mode;
		brightness=rgb_red_current_brightness;
		pwm_cfg = led->rgb_cfg->pwm_cfg;
		break;
	case QPNP_ID_RGB_GREEN:
		mode=rgb_green_mode;
		brightness=rgb_green_current_brightness;
		pwm_cfg = led->rgb_cfg->pwm_cfg;
		break;
	case QPNP_ID_RGB_BLUE:
		mode=rgb_blue_mode;
		brightness=rgb_blue_current_brightness;
		pwm_cfg = led->rgb_cfg->pwm_cfg;
		break;
	default:
		dev_err(&led->spmi_dev->dev,
			"Invalid LED id type for parameter\n");
		return -EINVAL;
	}

	if(pwm_cfg->blinking==true)blinking=1;
	else blinking=0;
    //start_idx=pwm_cfg->duty_cycles->start_idx;
    start_idx =pwm_cfg->lut_params.start_idx;  
    pause_lo=pwm_cfg->lut_params.lut_pause_lo;
    pause_hi = pwm_cfg->lut_params.lut_pause_hi;
    ramp_step_ms=pwm_cfg->lut_params.ramp_step_ms;
    lpg_pm_pwm_period=pwm_cfg->pwm_period_us;  //config RGB three pwm_period same value
    
    return sprintf(buf, "%d,%d,%d,%d,%d,%d,%d,%d\n",mode,brightness,blinking,start_idx,pause_lo,pause_hi,ramp_step_ms,lpg_pm_pwm_period);
}
#endif
...
#ifdef CONFIG_ZTEMT_RGB_BREATH_LEDS
static DEVICE_ATTR(user_mode, 0664, user_mode_show, user_mode_store);
#endif
...
#ifdef CONFIG_ZTEMT_WLED_CABC
/*mayu add 2013.9.22*/
    led->cabc_on = WLED_CABC_ENABLE_DEFAULT;
		rc = of_property_read_u32(node, "qcom,cabc", &val);
		if (!rc)
		  led->cabc_on = (u8) val;
#endif
