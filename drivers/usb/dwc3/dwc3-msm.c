...
#ifdef CONFIG_ZTEMT_NX504J_CHARGE_X
...
static ssize_t tx_ovrd_drv_lo_show_data(struct device *dev,
...
static ssize_t tx_ovrd_drv_lo_store_data(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	...
	data &= ~0x7F;
	data |= (0x7F | (1 << 14));
	dwc3_msm_ssusb_write_phycreg(mdwc->base, 0x1002, val);

	return 1;
}


static ssize_t tx_debug_show_data(struct device *dev,
			 struct device_attribute *attr,
			 char *buf)
{
    struct dwc3_msm *mdwc = dev_get_drvdata(dev);
...
static ssize_t tx_debug_store_data(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	u32 data = 0;
    struct dwc3_msm *mdwc = dev_get_drvdata(dev);
...
/*
	...
	data &= ~0xFF0;
	data |= 0x20;
	dwc3_msm_ssusb_write_phycreg(mdwc->base, 0x1010, val);
...
static DEVICE_ATTR(tx_ovrd_drv_lo, S_IRUGO, tx_ovrd_drv_lo_show_data, tx_ovrd_drv_lo_store_data);
static DEVICE_ATTR(tx_debug, S_IRUGO, tx_debug_show_data, tx_debug_store_data);




static struct attribute *dwc3_attrs[] = {
	&dev_attr_tx_ovrd_drv_lo.attr,
...
static int __devinit dwc3_msm_probe(struct platform_device *pdev)
{
	...
	//add by xuexiaojun
	ret = sysfs_create_group(&pdev->dev.kobj, &dwc3_attr_group);
	if (ret) {
...
err_release_sysfs_group:
		sysfs_remove_group(&pdev->dev.kobj, &dwc3_attr_group);
  //end add
put_xcvr:
