...
#define CYTTSP5_TPK_FIRMEARE_VERSION    0x0583
#define CYTTSP5_EELY_FIRMEARE_VERSION   0x0502
#define CYTTSP5_TPK_I_FIRMEARE_VERSION  0x05CC

#define CYTTSP5_GW_NX504_FIRMEARE_VERSION  0x0702
...
static int cyttsp_check_version(struct cyttsp5_sysinfo *si, struct device *dev)
{
    ...
    if (si->fw_ver_ic == 0x0584 || si->fw_ver_ic == 0x05A2
        || si->fw_ver_ic == 0x0000) {
        si->fw_ver_dr = CYTTSP5_TPK_I_FIRMEARE_VERSION;
    ...
    switch (si->fw_ver_ic & 0xFFF0) {
        case 0x05C0:
            ...
            break;
        case 0x0700:
            si->fw_ver_dr = CYTTSP5_GW_NX504_FIRMEARE_VERSION;
            memcpy(si->cy_fw_file_name, "cyttsp5_fw.bin", CY_FW_FILE_NAME_LEN);
            break;
        case 0x0580:
            si->fw_ver_dr = CYTTSP5_TPK_FIRMEARE_VERSION;
            memcpy(si->cy_fw_file_name, "cyttsp5_fw_t.bin", CY_FW_FILE_NAME_LEN);
            break;
        case 0x0500:
            si->fw_ver_dr = CYTTSP5_EELY_FIRMEARE_VERSION;
            memcpy(si->cy_fw_file_name, "cyttsp5_fw_e.bin", CY_FW_FILE_NAME_LEN);
            break;
        default:
            si->fw_ver_dr = 0x0000;
            memcpy(si->cy_fw_file_name, "cyttsp5_fw.bin", CY_FW_FILE_NAME_LEN);
