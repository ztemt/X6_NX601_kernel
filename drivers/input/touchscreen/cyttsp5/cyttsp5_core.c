		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/*Switch touch mode*/
#if 1
    /*03: Disable hover; 15: All feature*/
...
#if 0
    /*ZTEMT Added by luochangyang, 2014/01/05*/
	rc = cyttsp5_hid_output_get_param(cd, 0xd0, &cd->touch_mode);
	if (rc < 0)
		dev_err(cd->dev, "%s: cyttsp5_hid_output_get_param failed r=%d\n",
			__func__, rc);
    /*ZTEMT END*/
#endif
...
#if 0
    /*ZTEMT Added by luochangyang, 2014/01/05*/
    /*03: Disable hover, Auto Glove; 15: All feature*/
	rc = cyttsp5_hid_output_set_param(cd, 0xd0, cd->touch_mode);
	if (rc < 0)
...
static struct device_attribute attributes[] = {
	...
	__ATTR(hw_reset, 0664, cyttsp5_hw_reset_show, NULL),
