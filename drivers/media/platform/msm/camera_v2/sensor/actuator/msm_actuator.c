	...
	struct reg_settings_t *settings)
{
	...
	if ((a_ctrl->act_device_type == MSM_CAMERA_PLATFORM_DEVICE) && \
		...
		//rc = Regwrite8byte(0x20, 0x08010103, 0x20000000, a_ctrl);
	...
	struct msm_actuator_move_params_t *move_params)
{
	...
	//printk("ssss sid=%x\n", a_ctrl->i2c_client.cci_client->sid<<1);
	CDBG("ssss called, dir %d, num_steps %d\n", dir, num_steps);
...
#if 0//def CONFIG_IMX135_GBAO_LC898122
	if (a_ctrl->i2c_client.cci_client->sid == 0x48 >> 1) {	
	...
	}else if (a_ctrl->i2c_client.cci_client->sid == 0x32 >> 1) {
#else
