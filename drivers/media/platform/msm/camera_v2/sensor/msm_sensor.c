...
#endif
#if defined(CONFIG_IMX135) || defined(CONFIG_IMX135_GBAO)
...
int32_t msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_VREG:
			...
			if ((index == 0) && (power_setting_array->set_rear_vreg_gpio))
				power_setting_array->set_rear_vreg_gpio(1);
			if ((index == 0) && (power_setting_array->set_front_vreg_gpio))
				power_setting_array->set_front_vreg_gpio(1);
#endif
...
#if defined(CONFIG_IMX135) || defined(CONFIG_IMX135_GBAO) || defined(CONFIG_IMX135_GBAO_LC898122)
...
#if defined(CONFIG_ZTE_CAMERA_Z7) || defined(CONFIG_ZTE_CAMERA_Z7MINI)
			if ((index == 0) && (power_setting_array->set_rear_vreg_gpio))
				power_setting_array->set_rear_vreg_gpio(0);
			if ((index == 0) && (power_setting_array->set_front_vreg_gpio))
				power_setting_array->set_front_vreg_gpio(0);
#endif
			break;
		case SENSOR_I2C_MUX:
			if (data->i2c_conf && data->i2c_conf->use_i2c_mux)
...
#if 0//def CONFIG_IMX135
		if ((!strncmp(s_ctrl->sensordata->sensor_name, "imx135", 32)) && (reg_setting[0].reg_data == 0x13)) {
			printk("sensor name= %s\n", s_ctrl->sensordata->sensor_name);
			IMX135_update_wb_register_from_otp(s_ctrl);
		}
		if (!strncmp(s_ctrl->sensordata->sensor_name, "imx135", 32)) {
			printk("sensor name= %s\n", s_ctrl->sensordata->sensor_name);
			ofei_imx135_read_test(s_ctrl);
		}
#endif
		kfree(reg_setting);
		break;
...
static struct zte_vdd_gpio rear_vdd_gpio;
static struct zte_vdd_gpio front_vdd_gpio;

static int32_t msm_sensor_get_vreg_gpio_dt_data(struct device_node *of_node,
	struct msm_sensor_ctrl_t *s_ctrl)
{
	/* vcm-vdd-gpio */
	rear_vdd_gpio.vdd_gpio[REAR_VCM_VDD_GPIO] = of_get_named_gpio(of_node, "vcm-vdd-gpio", 0);
	CDBG("sss %s vcm-vdd-gpio=%d\n", __func__, rear_vdd_gpio.vdd_gpio[REAR_VCM_VDD_GPIO]);
	if (rear_vdd_gpio.vdd_gpio[REAR_VCM_VDD_GPIO] < 0) {
		pr_err("%s: Can't get vcm_vdd_gpio\n", __func__);
		return -EINVAL;
	}
	rear_vdd_gpio.num_gpio = 1;
	/* dvdd_gpio */
	rear_vdd_gpio.vdd_gpio[REAR_DVDD_GPIO] = of_get_named_gpio(of_node, "dvdd-gpio", 0);
	CDBG("sss %s dvdd-gpio=%d\n", __func__, rear_vdd_gpio.vdd_gpio[REAR_DVDD_GPIO]);
	if (rear_vdd_gpio.vdd_gpio[REAR_DVDD_GPIO] < 0) {
		pr_err("%s: Can't get dvdd_gpio\n", __func__);
		return -EINVAL;
	}
	rear_vdd_gpio.num_gpio++;
	return 0;
}
int msm_sensor_set_vreg_gpio_dt_data(int enable)
{
	int ret = 0;
	static bool once = true;
	int i;
	if (once) {
		ret = gpio_request(rear_vdd_gpio.vdd_gpio[REAR_VCM_VDD_GPIO], "vcm_vdd_gpio");
		if (ret) {
			pr_err("%s : failed to request gpio %d\n", __func__,
					rear_vdd_gpio.vdd_gpio[REAR_VCM_VDD_GPIO]);
			goto err0;
		}
		ret = gpio_request(rear_vdd_gpio.vdd_gpio[REAR_DVDD_GPIO], "dvdd_gpio");
		if (ret) {
			pr_err("%s : failed to request gpio %d\n", __func__,
					rear_vdd_gpio.vdd_gpio[REAR_DVDD_GPIO]);
			goto err1;
		}
		once = false;
	}
	if (enable) {
		for (i = 0; i < rear_vdd_gpio.num_gpio; i++) {
			gpio_direction_output(rear_vdd_gpio.vdd_gpio[i], enable);
			CDBG("sss i=%d %d=%d\n",i, rear_vdd_gpio.vdd_gpio[i],gpio_get_value(rear_vdd_gpio.vdd_gpio[i]));
		}
	} else {
		for (i = (rear_vdd_gpio.num_gpio-1); i >= 0; i--) {
			gpio_set_value_cansleep(rear_vdd_gpio.vdd_gpio[i], enable);
			CDBG("sss %d=%d\n",rear_vdd_gpio.vdd_gpio[i],gpio_get_value(rear_vdd_gpio.vdd_gpio[i]));
		}
	}
	return ret;
	err1:
	gpio_free(rear_vdd_gpio.vdd_gpio[REAR_DVDD_GPIO]);
	err0:
	gpio_free(rear_vdd_gpio.vdd_gpio[REAR_VCM_VDD_GPIO]);
	return ret;
}

static int32_t msm_sensor_get_front_vreg_gpio_dt_data(struct device_node *of_node,
	struct msm_sensor_ctrl_t *s_ctrl)
{
	/* avdd_gpio */
	front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO] = of_get_named_gpio(of_node, "avdd-gpio", 0);
	CDBG("sss %s avdd_gpio=%d\n", __func__, front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO]);
	if (front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO] < 0) {
		pr_err("%s: Can't get avdd_gpio\n", __func__);
		return -EINVAL;
	}
	return 0;
}

int msm_sensor_set_front_vreg_gpio(int enable)
{
	int ret = 0;
	static int once = true;
	if (once) {
		ret = gpio_request(front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO], "avdd_gpio");
		if (ret) {
			pr_err("%s : failed to request gpio %d\n", __func__,
					front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO]);
			goto err0;
		}
		once = false;
	}
	...
	if (enable) {
		gpio_direction_output(front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO], enable);

	} else {
		gpio_set_value_cansleep(front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO], enable);
	}
		
	CDBG("sss %d=%d\n",front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO],gpio_get_value(front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO]));
	return ret;
	err0:
	gpio_free(front_vdd_gpio.vdd_gpio[FRONT_AVDD_GPIO]);
	return ret;
}

#endif

#ifdef CONFIG_ZTE_CAMERA_Z7MINI
struct zte_vdd_gpio {
	int vdd_gpio[2];
	int num_gpio;
};
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	/* avdd_gpio */
	rear_vdd_gpio.vdd_gpio[REAR_AVDD_GPIO] = of_get_named_gpio(of_node, "avdd-gpio", 0);
	printk("sss %s avdd-gpio=%d\n", __func__, rear_vdd_gpio.vdd_gpio[REAR_AVDD_GPIO]);
	if (rear_vdd_gpio.vdd_gpio[REAR_AVDD_GPIO] < 0) {
		pr_err("%s: Can't get dvdd_gpio\n", __func__);
		return -EINVAL;
	}
	rear_vdd_gpio.num_gpio = 1;
...
int msm_sensor_set_vreg_gpio_dt_data(int enable)
{
	...
	int i;
	if (once) {
		ret = gpio_request(rear_vdd_gpio.vdd_gpio[REAR_AVDD_GPIO], "avdd_gpio");
		if (ret) {
			pr_err("%s : failed to request gpio %d\n", __func__,
					rear_vdd_gpio.vdd_gpio[REAR_AVDD_GPIO]);
			goto err1;
		}
		once = false;
	}
	...
	return ret;
	err1:
	gpio_free(rear_vdd_gpio.vdd_gpio[REAR_AVDD_GPIO]);
	return ret;
}
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
	...
	if (pdev->dev.of_node) {
		...
		if (s_ctrl->power_setting_array.get_rear_dt_vreg_gpio){
			msm_sensor_get_vreg_gpio_dt_data(pdev->dev.of_node, s_ctrl);
			if (rc < 0) {
				pr_err("%s failed line %d\n", __func__, __LINE__);
				return rc;
			}
		}
		if (s_ctrl->power_setting_array.get_front_dt_vreg_gpio){
			msm_sensor_get_front_vreg_gpio_dt_data(pdev->dev.of_node, s_ctrl);
			if (rc < 0) {
				pr_err("%s failed line %d\n", __func__, __LINE__);
				return rc;
			}
