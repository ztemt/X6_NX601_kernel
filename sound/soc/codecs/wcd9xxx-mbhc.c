...
#if defined (CONFIG_ZTEMT_AUDIO_Z7) || defined (CONFIG_ZTEMT_AUDIO_Z7MINI)
...
//choose the mothod of close reg 0x1AB to avoid noise
/*		
#ifdef CONFIG_ZTEMT_AUDIO_Z7MINI
			pr_debug("[ZTEMT]%s: if external loudspeaker is on, then enable NCP\n",
						 __func__); 	
		    pr_debug("[ZTEMT]get reg 0x1ca value.......... %x\n",snd_soc_read(codec,0x1CA));
			if(snd_soc_read(codec,0x1CA)&0x0F){
			pr_debug("ZTEMT  ZTEMT-----------OPEN AMP");
			snd_soc_update_bits(codec, WCD9XXX_A_NCP_EN,0x01, 0x01);
...
/*       
        #ifdef CONFIG_ZTEMT_AUDIO_Z7MINI
		if(snd_soc_read(codec,0x1CA)&0x0F)gpio_direction_output(16,0);//close speaker amp
        #endif
*/		
        /* Close the NCP for enabling the earphone */
		
//      snd_soc_update_bits(codec, WCD9XXX_A_NCP_EN,0x01, 0x00);
		//snd_soc_update_bits(codec, 0x1AB,0x30, 0x00);
/*add 300ms delays for headphone dectection  */
        msleep(300);  
        gpio_direction_output(COMPATIBLE_HEADSET_DETECTION_GPIO,0);
        msleep(100);
